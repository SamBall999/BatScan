...
<img src="Diagrams/block_diagram.png" alt="Block diagram showing interaction of various modules in the sonar system." width="550"/>
...
signal using the ADC. The sampled received signal is then sent to the **PC** for signal processing. 
...
### Receiver
In order to read in the received signal from the receiver transducer, the 16-bit ADC on the Teensy board is used to convert the analogue input to a digital representation which can then be processed, as shown below.

<img src="Diagrams/receiver.png" alt="Receiver chain including ADC and microcontroller." width="450"/>

The _LibSerialPort.jl_ library is utilised to allow the signal processing algorithm to access the values read in by the microcontroller via the serial port.
